...
`get_codex_quota` 支持四种模式：`5m`、`15m`、`30m` 分别返回对应滑动窗口；`conversation_start` 在新对话开始时使用一次，绕过低频门控并返回完整的 5/15/30 分钟短报。

Agent 应在每次新对话开始时调用一次 `mode=conversation_start`；持续工作时再按需使用三种时间窗口模式。
